...
export const merchantGetMiddleware = async (c: Context, next: Next) => {
  ...
  const isAllowed = await rateLimit(
    `rate-limit:${teamMemberProfile.company_member_id}:merchant-get`,
    50,
    "1m",
    c
  );
...
export const merchantDeleteMiddleware = async (c: Context, next: Next) => {
  ...
  const isAllowed = await rateLimit(
    `rate-limit:${teamMemberProfile.company_member_id}:merchant-delete`,
    50,
    "1m",
    c
  );
...
export const merchantPostMiddleware = async (c: Context, next: Next) => {
  ...
  const isAllowed = await rateLimit(
    `rate-limit:${teamMemberProfile.company_member_id}:merchant-post`,
    50,
    "1m",
    c
  );
...
export const merchantPatchMiddleware = async (c: Context, next: Next) => {
  ...
  const isAllowed = await rateLimit(
    `rate-limit:${teamMemberProfile.company_member_id}:merchant-patch`,
    50,
    "1m",
    c
  );
...
export const merchantBankMiddleware = async (c: Context, next: Next) => {
  ...
  const isAllowed = await rateLimit(
    `rate-limit:${teamMemberProfile.company_member_id}:merchant-bank-get`,
    50,
    "1m",
    c
  );
...
export const merchantBalanceMiddleware = async (c: Context, next: Next) => {
  ...
  const isAllowed = await rateLimit(
    `rate-limit:${teamMemberProfile.company_member_id}:merchant-balance-history`,
    50,
    "1m",
    c
  );
