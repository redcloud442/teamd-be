...
export const dashboardPostMiddleware = async (c, next) => {
    ...
    const isAllowed = await rateLimit(`rate-limit:${teamMemberProfile.company_member_id}:dashboard-post`, 100, "1m", c);
    if (!isAllowed) {
...
export const dashboardGetMiddleware = async (c, next) => {
    ...
    const isAllowed = await rateLimit(`rate-limit:${teamMemberProfile.company_member_id}:dashboard-get`, 100, "1m", c);
    if (!isAllowed) {
        return sendErrorResponse("Too Many Requests", 429);
    }
    await next();
