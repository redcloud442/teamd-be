...
export const referralIndirectModelPost = async (params) => {
    ...
    const hierarchyResult = await prisma.$queryRaw `
      ...
      WHERE ar.company_referral_hierarchy LIKE ${"%." + teamMemberProfile.company_member_id + ".%"}
    `;
    const allDownlineIds = hierarchyResult.map((r) => r.company_referral_member_id);
    const finalIndirectReferralIds = allDownlineIds.filter((id) => !directReferralIds.includes(id));
    ...
    LIMIT $2 OFFSET $3
    `, finalIndirectReferralIds, limit, offset);
