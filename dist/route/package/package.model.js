...
export const packageDailytaskGetModel = async (params) => {
    ...
    if (bountyLogs.length === 0)
        return;
    const memberIds = Array.from(new Set(bountyLogs.map((log) => log.package_ally_bounty_member_id)));
    const startDate = getPhilippinesTime(new Date(), "start");
    await prisma.$transaction(async (tx) => {
        await tx.alliance_wheel_table.createMany({
        ...
        const wheelData = await tx.alliance_wheel_table.findMany({
            where: {
      ...
      `;
        ...
        const referralCountMap = new Map(referralCounts.map((r) => [
        ...
        if (transactions.length > 0) {
            await tx.alliance_transaction_table.createMany({ data: transactions });
        }
        if (updates.length > 0) {
            await tx.$executeRaw `
    ...
    `;
        ...
        if (spinCounts.length > 0) {
            await Promise.all(spinCounts.map(({ memberId, spinCount }) => tx.alliance_wheel_log_table.upsert({
